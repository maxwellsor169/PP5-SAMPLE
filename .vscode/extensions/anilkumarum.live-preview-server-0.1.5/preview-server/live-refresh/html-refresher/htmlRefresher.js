...
export default class HtmlRefresher extends HtmlUpdater {
	...
	getElemDataAtOffset(change) {
		...
		if (text.charCodeAt(0) === CharCode.LineBreak || text.charCodeAt(0) === CharCode.Tab) {
			//TEST need to test (don't shift node if rangeoffset > node.end )
			this.shiftTree(node, text.length), NodelinkList.shiftParent(node, rangeOffset, text.length);
			return null;
		}
		//add new element
		...
		if (text.endsWith('""')) {
			const attrData = text !== '""' ? this.#getAttrTxt(change) : { start: rangeOffset, text };
